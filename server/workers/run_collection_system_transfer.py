...
def build_ssh_filelist(gearman_worker, source_dir): # pylint: disable=too-many-branches,too-many-statements,too-many-locals
    ...
    command = ['rsync', '-r', '--protect-args', '-e', 'ssh', gearman_worker.collection_system_transfer['sshUser'] + '@' + gearman_worker.collection_system_transfer['sshServer'] + ':' + source_dir.replace(" ", "\\ ") + '/'] if gearman_worker.collection_system_transfer['sshUseKey'] == '1' else ['sshpass', '-p', gearman_worker.collection_system_transfer['sshPass'], 'rsync', '-r', '--protect-args', '-e', 'ssh', gearman_worker.collection_system_transfer['sshUser'] + '@' + gearman_worker.collection_system_transfer['sshServer'] + ':' + source_dir.replace(" ", "\\ ") + '/']
    logging.debug("Command: %s", ' '.join(command))
...
def transfer_ssh_source_dir(gearman_worker, gearman_job): # pylint: disable=too-many-locals
    ...
    command = ['rsync', '-tri', bandwidth_limit, '--protect-args', '--files-from=' + ssh_filelist_filepath, '-e', 'ssh', gearman_worker.collection_system_transfer['sshUser'] + '@' + gearman_worker.collection_system_transfer['sshServer'] + ':' + source_dir.replace(' ', '\\ '), dest_dir] if gearman_worker.collection_system_transfer['sshUseKey'] == '1' else ['sshpass', '-p', gearman_worker.collection_system_transfer['sshPass'], 'rsync', '-tri', '--protect-args', bandwidth_limit, '--files-from=' + ssh_filelist_filepath, '-e', 'ssh', gearman_worker.collection_system_transfer['sshUser'] + '@' + gearman_worker.collection_system_transfer['sshServer'] + ':' + source_dir.replace(" ", "\\ "), dest_dir]
    ...
    def on_job_execute(self, current_job):
        ...
        # if temporal bands are used then set to specified bounds for the corresponding cruise/lowering
            ...
            if self.collection_system_transfer['cruiseOrLowering'] == "0":
                ...
                self.data_end_date = self.ovdm.get_cruise_end_date() or "9999/12/31 23:59"
                ...
                self.data_end_date = self.ovdm.get_lowering_end_date() or "9999/12/31 23:59"
